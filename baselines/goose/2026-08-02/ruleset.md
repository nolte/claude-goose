...
## What changed from 2026-08-01

| Change | Detail |
|---|---|
| **`GAP-EXT-SEMANTICS` narrowed** | Researched and found to have **no authoritative basis**. The gap remains, but is now "searched, nothing found" rather than "not yet researched" |
| **New source `S-009`** | Records that search, so nobody repeats it blindly |
| Criteria R-001 … R-010 | **Unchanged.** No criterion was added — see below |

**No criterion was added, and that is the finding.** The documentation specifies extension field
*types* but no constraints on their *values*: `cmd` has "no formal specification of required format
or syntax", `timeout` no units, minimum, maximum or default, and `bundled` no operational meaning
beyond "whether the extension is bundled with goose".

Inventing a rule here — a plausible timeout ceiling, an expected `cmd` shape — is exactly what this
project forbids. The honest outcome of research that finds nothing is a better-documented gap, not a
criterion.
