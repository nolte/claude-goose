# Ruleset — Revision 2026-07-31b

**Goose version**: v1.45.0 | **Format**: see `criterion-format.md` in this directory
**Supersedes**: `2026-07-31` (retained, unmodified)

Every criterion below traces to a record in `sources.md`. Criteria are ordered by id.

## What changed from 2026-07-31

| Change | Detail |
|---|---|
| **Added `R-008`** | `inline_python` extensions must declare `code`, `timeout`, `description` |
| **Added `R-009`** | `stdio` extensions must declare `cmd`, `args`, `timeout` |
| **Added `R-010`** | A recipe must declare `prompt` to be runnable headlessly |
| **Narrowed `GAP-EXT-INTERNALS`** | Per-type required fields are now covered by R-008/R-009. The gap remains only for semantic correctness of those values |
| R-001 … R-007 | Unchanged in substance |

**`R-010` is the most consequential addition.** Like `R-002` it is not host-enforced, and unlike
`R-002` it was found by the process failing on itself: the review recipe declared `instructions` but
no `prompt`, loaded cleanly, and then died at run time with "no text provided for prompt in headless
mode". A recipe with this defect passes every validation and fails only when someone tries to
automate it.
