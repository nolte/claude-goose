...
| `GAP-EXT-SEMANTICS` | **Semantic correctness of extension field values** | `R-008`/`R-009` now cover which fields must be *present*. Whether a `cmd` is sensible, a `timeout` adequate, or inline `code` safe is unresearched | The subject declares at least one extension |
