...
- [X] T030 [US2] Verify protection is restored after deletion through the platform UI (`SC-007`) — **measured, not inspected.** Protection on `develop` was deleted (`DELETE /branches/develop/protection`, endpoint then returned `Branch not protected`), `.github/settings.yml` was touched via PR #4, and protection returned within 20s of the merge. All seven captured fields identical to the pre-deletion baseline: both required contexts, `strict`, `required_linear_history`, `allow_force_pushes`, `allow_deletions`, `enforce_admins`, and the null review requirement
...
- [X] T052 Verify no file hashed in `.specify/integrations/*.json` was hand-edited (`FR-033`) — verified by recomputing SHA256 over all 20 manifest entries (10 in `claude.manifest.json`, 10 in `speckit.manifest.json`): zero drift, zero missing. Made permanent rather than a one-off: added the `manifest-integrity` gate class (`scripts/check_manifest_integrity.py`), the only hook with `always_run: true`, and demonstrated it failing on a deliberately edited vendored file per `FR-010`/`SC-002`. Now also a standing check class — `manifest-integrity` runs `always_run: true` in the gate, so this is enforced continuously rather than verified once, and demonstrated failing on a deliberately edited vendored file as `FR-010`/`SC-002` require of every class
- [X] T053 Update `CLAUDE.md` with the gate entry point, the pinned shared-workflow version, and the rule that checks live in `.pre-commit-config.yaml` rather than inline in workflows (`FR-032`) — added a "CI/CD, branches, and releases" section; also corrected the now-false claim "There is no build or lint step"
